...
const {
  ...
  setVisualsText, getPosition, getFaceId, getAbilityBlock, setAbilityBlock,
} = require('./draftClassFile');
const { loadTemplateModel, DEFAULT_TARGET } = require('./draftClassTemplate');
const { createAppearanceAssigner, catalogFromTemplate } = require('./appearanceCatalog');
...
// Madden 26's file is a fixed 402 slots. Madden 27's is a different size (389 in
// the export this app's template was baked from), so the slot count is read from
// whichever template is actually loaded rather than assumed. This constant stays
// as the M26 value for callers that still want a headline number, but nothing in
// the build path uses it -- see slotCount inside buildDraftClassFile.
...
function applyPlayer(templatePlayer, generatedPlayer, appearanceAssigner, matchCollege, warn, gear, validAssets, faceStats) {
  ...
  // DevTrait, not TraitDevelopment. DevTrait is the Madden dev trait
  // assignDevTraits (pipeline.js) decided for this prospect and the Draft
  // Class page displays; TraitDevelopment is his COLLEGE performance tier,
  // carried over from the CFB save. They share one byte in the file, and
  // writing the college tier here is exactly the bug that shipped in 0.3.0 --
  // a player listed as Star imported as a Superstar, ignoring the configured
  // dev-trait percentages. See the two-vocabulary note in draftClassFile.js.
  if (generatedPlayer.DevTrait) {
    try { p = setDevTrait(p, generatedPlayer.DevTrait); }
    catch (e) { warn(`Could not set dev trait for ${generatedPlayer.FirstName} ${generatedPlayer.LastName}: ${e.message}`); }
  }
  ...
  //   1. assignExact    -- the player's OWN CFB head, when Madden ships it.
  //   2. assignByAssetName -- skin tone AND facial-hair family recovered from
  //      GenericHeadAssetName, when the player's own head isn't in Madden's
  //      catalog but their asset name still parses (most CFB-generated
  //      players; not the scanned Unique_* ones -- see appearanceCatalog.js).
  //   3. assign         -- skin tone only, oldest and least specific tier,
  //      for anyone the two above can't place at all.
  const look = appearanceAssigner.assignExact(generatedPlayer.PLYR_GENERICHEAD)
    || appearanceAssigner.assignByAssetName(generatedPlayer.GenericHeadAssetName)
    || appearanceAssigner.assign(generatedPlayer.SkinTone);
  if (faceStats) {
    if (look.ownHead) faceStats.ownHead++;
    else if (look.familyMatch) faceStats.familyMatch++;
    else faceStats.toneOnly++;
  }
  ...
  // M27's ability/trait block (draftClassFile.js's getAbilityBlock/
  // setAbilityBlock -- the struct's undocumented tail past offset 202, which
  // this file has no per-field writer for). Copied from the SAME
  // position-matched donor gear uses below, rather than left as whatever the
  // fill loop's slot happened to already contain -- that inherited-from-slot
  // behavior was the actual bug: measured position-mismatched on 204 of 224
  // players in a real class (M27_FIELD_FIXES_ROADMAP.md Phase 1). A no-op on
  // M26, which has no such region (getAbilityBlock returns null for both the
  // donor and the target slot there, since neither struct reaches byte 202).
  if (gear && gear.donor) {
    const donorBlock = getAbilityBlock(gear.donor);
...
// `options.target` selects which game's file to emit ('m26' | 'm27'). Each
// target has its own bundled template, and the template's own slot count drives
// the fill -- a Madden 27 file is not a Madden 26 file with a different tag.
function buildDraftClassFile(generatedClass, options = {}) {
  ...
  const model = loadTemplateModel(target);
  const slotCount = model.players.length;

  // Faces and gear are both validated against the TARGET game, not against
  // Madden 26's baked assets. For M26 that stays the rich baked catalog/list;
  // for any other target the template itself is the authority, since every
  // asset in a real export of that game is by definition one it ships.
  const isM26 = target === 'm26';
  const appearanceCatalog = isM26 ? null : catalogFromTemplate(model, { faceIdOf: getFaceId });
  const validAssets = isM26 ? loadValidAssets() : validAssetsFromTemplate(model);

  const sorted = generatedClass.slice().sort((a, b) => (a.DraftRank ?? Infinity) - (b.DraftRank ?? Infinity));
  const fillCount = Math.min(sorted.length, slotCount);
  const top = sorted.slice(0, fillCount);
  const appearanceAssigner = createAppearanceAssigner(appearanceCatalog);
  ...
  const faceStats = { ownHead: 0, familyMatch: 0, toneOnly: 0 };
  ...
  for (let i = 0; i < fillCount; i++) {
    ...
    model.players[i] = applyPlayer(
      ...
      { donor, stats: gearStats }, validAssets, faceStats,
    );
  }
  ...
  // Face coverage was previously invisible -- a class could come out almost
  // entirely substituted (or, before the skin-tone-distribution fix, skewed
  // to one tone) with nothing in the log to show it. This says so directly,
  // in the same three tiers applyPlayer actually tries.
  log(`Faces: ${faceStats.ownHead} of ${fillCount} players kept their own face, `
    + `${faceStats.familyMatch} matched on skin tone and facial-hair family, `
    + `${faceStats.toneOnly} matched on skin tone only.`);
  ...
  if (fillCount < slotCount) {
    const leftover = slotCount - fillCount;
    let msg = `Only ${fillCount} of the file's ${slotCount} slots were filled with your generated `
