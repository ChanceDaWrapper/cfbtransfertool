...
// Madden's own rookie Dev Trait scale (Normal/Star/Superstar/X-Factor), the
// thing assignDevTraits (pipeline.js) actually decides for a prospect and the
// Draft Class page displays. Deliberately separate from draftClassFile.js's
// TRAIT_DEVELOPMENT_ENUM, which names the SAME byte offset's raw values as
// they appear on a college player's own record (Normal/College_Impact/
// College_Star/College_Elite/Hidden) -- that enum exists to read a departing
// CFB player's own performance tier, not to describe what this app assigns a
// generated Madden prospect. The two enums are ordinally identical (index 0-3
// both mean "no dev, next tier, next tier, top tier"), which is what makes a
// raw-int write here safe and unambiguous.
const MADDEN_DEV_TRAIT_TO_RAW = { Normal: 0, Star: 1, Superstar: 2, XFactor: 3 };
...
function applyPlayer(templatePlayer, generatedPlayer, appearanceAssigner, matchCollege, warn, gear, validAssets) {
  ...
  const devRaw = MADDEN_DEV_TRAIT_TO_RAW[generatedPlayer.DevTrait];
  if (devRaw !== undefined) p = setDevTrait(p, devRaw);
  ...
  // Prefer the player's OWN CFB head when the destination game ships it --
  // Madden's head names are CFB's with a "gen_" prefix (same taxonomy; see
  // assignExact). Only when it isn't available do we substitute a same-tone
  // head from the catalog, which is a guess about what the player looks like
  // and, as a real M27 report showed, not even reliably the right skin.
...
function buildDraftClassFile(generatedClass, options = {}) {
  ...
  for (let i = 0; i < fillCount; i++) {
    ...
    model.players[i] = applyPlayer(
      ...
      { donor, stats: gearStats }, validAssets,
    );
  }
