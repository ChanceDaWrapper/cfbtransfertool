...
// The set of gear assets a given TARGET GAME is known to accept, derived from
// that game's own bundled template. data/maddenGearAssets.json was baked from
// Madden 26 and cannot vouch for Madden 27: writing an M26-only asset name into
// an M27 file would produce a missing item in-game, and nothing would surface
// that at export time.
//
// A real export of the target game is self-evidently a list of assets that game
// ships, so it is used as the authority for anything that is not M26. This is
// deliberately conservative -- Madden 27 certainly has more gear than the ~284
// assets one export happens to use, so some college items that could have
// carried over will instead keep the position-matched donor's. That is the safe
// direction to be wrong in: the player still looks right, just less personalised.
function validAssetsFromTemplate(model) {
  const set = new Set();
  for (const p of model.players) {
    const v = p.json && p.json.visuals;
    for (const lo of ((v && v.loadouts) || [])) {
      for (const el of (lo.loadoutElements || [])) {
        const n = el.itemAssetName;
        if (n && n.trim()) set.add(n);
      }
    }
  }
  return set;
