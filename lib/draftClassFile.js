...
// ---- version profiles -------------------------------------------------------
// Madden 27 keeps the same container, the same per-slot shape, and the same
// field ORDER as Madden 26 -- but it widened two things, measured against real
// exports of both games (research/probe47-draftclass-m27.js):
//
//   * the per-player binary record: 200 -> 244 bytes. The extra space holds five
//     u16 fields at record offsets 202/204/206/208/210 (plus a byte at 242),
//     populated for 389/389 players, values 0..63, near-unique within a player
//     and drawn from position-appropriate subsets -- i.e. per-player ability or
//     trait slots. They are carried through verbatim from the template donor
//     (see draftClassExporter), never synthesized.
//   * the FirstName field: 17 -> 21 bytes, which shifts every field after it by
//     exactly +4. Verified as the ONLY shift under which age/height/weight/
//     position are simultaneously plausible for all 389 players (100% vs ~1%
//     for the next best), and under which every rating byte lands 0..99.
//
// Unchanged: position enum, dev-trait enum, the rating set and its order, the
// draft round/pick convention, and the JSON visuals blob's role.
const FORMATS = {
  m26: { key: 'm26', binaryRecordLen: 200, fieldShift: 0 },
  m27: { key: 'm27', binaryRecordLen: 244, fieldShift: 4 },
};
function detectFormat(schemaTag) {
  return String(schemaTag || '').startsWith('Madden-27-') ? FORMATS.m27 : FORMATS.m26;
}

// Every field offset in this module is expressed in MADDEN 26 coordinates; this
// rebases one onto whatever format the player actually came from. Reading
// through here (rather than indexing raw directly) is what makes the same
// getters/setters work for both games.
function off(player, maddenBase) {
  return maddenBase + ((player.binary && player.binary.fieldShift) || 0);
}
...
function parseDraftClassFile(buf) {
  ...
  // Picking the record length from the schema tag is what lets a Madden 27 file
  // parse at all: with M26's 200 the cursor lands 44 bytes short of the next
  // player's JSON and the walk dies on the first record boundary.
  const format = detectFormat(header.schemaTag);
  const BINARY_RECORD_LEN = format.binaryRecordLen;
  ...
  for (let i = 0; i < header.playerCount; i++) {
    ...
    players.push({
      ...
      binary: {
        ...
        fieldShift: format.fieldShift,
        // Which format this record came from. fieldShift alone is not enough:
        // the portrait ID is the one field that does NOT take the uniform
        // shift (see FACE_ID_OFFSET_BY_FORMAT), so its accessors need to know
        // the actual format, not just how far everything else moved.
  ...
  return { header, players, trailer, format, totalLength: buf.length };
}
...
// Replaces a player's ENTIRE visuals JSON with `newText`, re-padding so the
// slot's [json+gap] total is unchanged. Same invariant replaceJsonToken keeps,
// but for a rewrite too structural to express as one substring swap -- the gear
// translation in lib/gearTranslation.js swaps many item names at once and can
// change length in both directions at several places.
//
// The slack is real and generous: [json+gap] is a fixed 4096 bytes per player in
// every export checked (the bundled template plus two real Madden exports, 1206
// slots, zero exceptions), against a largest-observed JSON of 2477 -- so ~1600
// bytes spare on the tightest slot. Still checked rather than assumed.
//
// Takes TEXT, not an object, on purpose. These blobs cannot survive a
// JSON.parse -> JSON.stringify round trip: 29 of the template's 402 carry float
// literals ("baseBlend":1.0) that stringify renders as 1, changing bytes for no
// reason. Callers edit the original characters and hand the result back here.
function setVisualsText(player, newText) {
  let visuals;
  try {
    visuals = JSON.parse(newText);
  } catch (e) {
    throw new Error(`setVisualsText: replacement is not valid JSON (${e.message})`);
  }
  const newRaw = Buffer.from(newText, 'utf8');
  if (newRaw.includes(0)) {
    // A NUL inside the blob would terminate it early and swallow the padding
    // run the parser uses to find the next record.
    throw new Error('setVisualsText: replacement contains a NUL byte');
  }
  const slot = player.json.raw.length + player.gap.raw.length;
  const newGapLen = slot - newRaw.length;
  if (newGapLen < 0) {
    throw new Error(`setVisualsText: replacement is ${newRaw.length} bytes, ${-newGapLen} over the ${slot}-byte slot`);
  }
  return {
    ...player,
    json: { raw: newRaw, visuals },
    gap: { raw: Buffer.alloc(newGapLen, 0) },
  };
}
...
function getCharacterBuild(player) { return player.binary.raw[off(player, CHARACTER_BUILD_OFFSET)]; }
function setCharacterBuild(player, bodyType) {
  ...
  return setBinaryBytes(player, [{ offset: off(player, CHARACTER_BUILD_OFFSET), value: idx }]);
}
...
// THE ONE FIELD THAT DOES NOT TAKE THE UNIFORM +4 SHIFT. Everything else after
// FirstName moves by exactly +4 in Madden 27; the portrait ID moves by +2, so
// it lives at 148 there, not 150.
//
// Measured, not assumed. Scanning every u16 offset in M27's 244-byte record for
// M26's portrait signature (values in the 3347-4287 render band) returns exactly
// one candidate -- offset 148: 329/389 in band, 188 distinct values, range
// 3350-10354. Offset 150, where the uniform shift would have put it, holds only
// 12 distinct values in 0..13, which is plainly a different (small enum) field.
//
// The clincher is that M27 pairs each generic head with exactly ONE portrait:
// 188 distinct heads, 188 distinct values at 148, zero heads mapping to two
// portraits and zero portraits shared by two heads -- a perfect bijection. A
// field that is 1:1 with the head asset is the portrait; a 0..13 enum is not.
//
// Consequence of having had this wrong (M27_FIELD_FIXES_ROADMAP.md Phase 3):
// every M27 export read the portrait from 150 (garbage, ~0), then wrote ~0 back
// to 150 -- clobbering that small enum -- while never touching the real
// portrait at 148. Each exported player therefore kept whatever portrait the
// template slot they overwrote happened to carry, unrelated to the head (and so
// the skin) written on top of it. That is the "skin tones are completely
// screwed up in M27" report: the 3D head was right, the 2D portrait beside it
// was somebody else's.
const FACE_ID_OFFSET_BY_FORMAT = { m26: 146, m27: 148 };
function faceIdOffset(player) {
  const key = player && player.binary && player.binary.formatKey;
  const explicit = key && FACE_ID_OFFSET_BY_FORMAT[key];
  // Fall back to the uniform shift for any format not listed, so a future
  // version parses (wrongly, but harmlessly) rather than throwing here.
  return explicit !== undefined ? explicit : off(player, FACE_ID_OFFSET);
}
function getFaceId(player) {
  const b = player.binary.raw;
  const o = faceIdOffset(player);
  return b[o] + b[o + 1] * 256;
}
function setFaceId(player, faceId) {
  const o = faceIdOffset(player);
  return setBinaryBytes(player, [
    { offset: o, value: faceId & 0xff },
    { offset: o + 1, value: (faceId >> 8) & 0xff },
  ]);
}
...
function getCollegeIndex(player) {
  const b = player.binary.raw;
  return b[off(player, COLLEGE_INDEX_OFFSET)] + b[off(player, COLLEGE_INDEX_OFFSET) + 1] * 256;
}
function setCollegeIndex(player, collegeIndex) {
  return setBinaryBytes(player, [
    { offset: off(player, COLLEGE_INDEX_OFFSET), value: collegeIndex & 0xff },
    { offset: off(player, COLLEGE_INDEX_OFFSET) + 1, value: (collegeIndex >> 8) & 0xff },
  ]);
}
...
function getPosition(player) {
  const v = player.binary.raw[off(player, POSITION_OFFSET)];
...
function setPosition(player, position) {
  ...
  return setBinaryBytes(player, [{ offset: off(player, POSITION_OFFSET), value }]);
}
...
function getHeight(player) { return player.binary.raw[off(player, HEIGHT_OFFSET)]; }
function setHeight(player, heightInches) {
  ...
  return setBinaryBytes(player, [{ offset: off(player, HEIGHT_OFFSET), value: heightInches }]);
}
function getWeight(player) { return player.binary.raw[off(player, WEIGHT_OFFSET)] + WEIGHT_RAW_ADJUST; }
function setWeight(player, weightLbs) {
  ...
  return setBinaryBytes(player, [{ offset: off(player, WEIGHT_OFFSET), value: raw }]);
}
...
const DEVTRAIT_OFFSET = 140;    // dev trait -- see the two-vocabulary note below
...
// ONE BYTE, TWO VOCABULARIES. Offset 140 holds the same small integer in both
// games, and the games agree on what each value MEANS ordinally (0 = no
// development, rising to the top tier) -- but they name those values
// differently, because they are describing different things:
//
//   value | CFB 27 (TraitDevelopment)  | Madden (Dev Trait)
//   ------|---------------------------|-------------------
//     0   | Normal                    | Normal
//     1   | College_Impact            | Star
//     2   | College_Star              | Superstar
//     3   | College_Elite             | X-Factor
//     4   | Hidden                    | (no equivalent)
//
// A CFB player's value describes HOW HE PERFORMED IN COLLEGE. A Madden
// prospect's value is the dev trait he carries INTO the NFL. Those are
// genuinely different facts about a player, and this app assigns the second
// one itself (see assignDevTraits in pipeline.js) rather than copying the
// first.
//
// This collision caused a real shipped bug: the draft-class exporter reached
// for the field name that was in scope -- the CFB tier -- and wrote it into
// the export, so a player shown as Star in the app could import into Madden
// as a Superstar, ignoring the configured dev-trait percentages entirely.
// Only one vocabulary had a name here at the time, so nothing about the call
// looked wrong.
//
// Both are now named, and setDevTrait accepts EITHER. The two name sets
// overlap only at 'Normal', where they agree on 0, so resolution is
// unambiguous -- whichever vocabulary a caller reaches for, the byte written
// is the right one.
const TRAIT_DEVELOPMENT_ENUM = { 0: 'Normal', 1: 'College_Impact', 2: 'College_Star', 3: 'College_Elite', 4: 'Hidden' };
const MADDEN_DEV_TRAIT_ENUM = { 0: 'Normal', 1: 'Star', 2: 'Superstar', 3: 'XFactor' };

// name -> value, both vocabularies. Built from the tables above so the two can
// never drift apart from each other.
const DEV_TRAIT_NAME_TO_VALUE = (() => {
  const out = {};
  for (const [v, n] of Object.entries(TRAIT_DEVELOPMENT_ENUM)) out[n] = Number(v);
  for (const [v, n] of Object.entries(MADDEN_DEV_TRAIT_ENUM)) {
    if (n in out && out[n] !== Number(v)) {
      throw new Error(`dev-trait vocabularies disagree on "${n}": ${out[n]} vs ${v}`);
    }
    out[n] = Number(v);
  }
  return out;
...
function getAge(player) { return player.binary.raw[off(player, AGE_OFFSET)]; }
function setAge(player, age) {
  if (!Number.isInteger(age) || age < 0 || age > 255) throw new Error(`setAge: ${age} is not a valid raw byte value`);
  return setBinaryBytes(player, [{ offset: off(player, AGE_OFFSET), value: age }]);
}
// M27-ONLY. The struct's entire undocumented tail -- everything from offset
// 202 to the end of the record. Absolute, NOT run through off(): every other
// offset in this file is a MADDEN 26 coordinate rebased by fieldShift because
// the same field exists in both formats at different byte positions; this
// region has no M26 equivalent at all (M26's record ends at byte 200), so
// there is nothing to rebase FROM.
//
// Only the first 10 bytes are individually decoded (five u16 "ability/trait"
// values at 202/204/206/208/210 -- see the FORMATS comment above), and even
// those aren't understood well enough to justify five separate setters. The
// remaining ~30 bytes past that are unexplored and, for all this code knows,
// could carry more position-correlated data than the five that were actually
// measured. Copying the whole span unparsed is deliberately more conservative
// than writing five u16 setters and silently leaving an unknown remainder as
// an accidental leftover from whoever the template originally put in that
// slot -- which is exactly the class of bug this exists to close (see
// M27_FIELD_FIXES_ROADMAP.md Phase 1: measured at 204/224 players on a real
// class inheriting another position's block before this existed).
const ABILITY_BLOCK_START = 202;

// null on any format shorter than the block (M26's 200-byte record never
// reaches offset 202) -- callers use this to detect "nothing to copy" rather
// than receiving a zero-length or garbage slice.
function getAbilityBlock(player) {
  const raw = player && player.binary && player.binary.raw;
  if (!raw || raw.length <= ABILITY_BLOCK_START) return null;
  return raw.slice(ABILITY_BLOCK_START);
}

// Copies `block` (as returned by getAbilityBlock, from some OTHER player) onto
// `player`'s own tail. Silently a no-op if either side has no such region --
// exporting to M26 must never throw over a field M26 simply doesn't have.
function setAbilityBlock(player, block) {
  const raw = player && player.binary && player.binary.raw;
  if (!block || !raw || raw.length <= ABILITY_BLOCK_START) return player;
  const buf = Buffer.from(raw);
  block.copy(buf, ABILITY_BLOCK_START, 0, Math.min(block.length, buf.length - ABILITY_BLOCK_START));
  return { ...player, binary: { ...player.binary, raw: buf } };
}

function getJersey(player) { return player.binary.raw[off(player, JERSEY_OFFSET)]; }
function setJersey(player, num) {
  if (!Number.isInteger(num) || num < 0 || num > 99) throw new Error(`setJersey: ${num} is out of range (0-99)`);
  return setBinaryBytes(player, [{ offset: off(player, JERSEY_OFFSET), value: num }]);
}
...
function getArchetype(player) {
  const v = player.binary.raw[off(player, ARCHETYPE_OFFSET)];
...
function setArchetype(player, playerType) {
  ...
  return setBinaryBytes(player, [{ offset: off(player, ARCHETYPE_OFFSET), value }]);
}
...
function getDraftRound(player) { return player.binary.raw[off(player, DRAFT_ROUND_OFFSET)]; }
function setDraftRound(player, round) {
  ...
  return setBinaryBytes(player, [{ offset: off(player, DRAFT_ROUND_OFFSET), value }]);
}
function getDraftPick(player) {
  const b = player.binary.raw;
  return b[off(player, DRAFT_PICK_OFFSET)] + b[off(player, DRAFT_PICK_OFFSET) + 1] * 256;
}
function setDraftPick(player, pick) {
  ...
  return setBinaryBytes(player, [
    { offset: off(player, DRAFT_PICK_OFFSET), value: pick & 0xff },
    { offset: off(player, DRAFT_PICK_OFFSET) + 1, value: (pick >> 8) & 0xff },
  ]);
}

// Returns the raw value plus BOTH readings of it (see the two-vocabulary note
// above). `name` stays the CFB tier so existing callers are unaffected;
// `maddenName` is the same byte read as a Madden dev trait. A caller that
// cares which one it means should say so by picking a field, rather than
// getting whichever this function happened to choose.
function getDevTrait(player) {
  const v = player.binary.raw[off(player, DEVTRAIT_OFFSET)];
  return {
    value: v,
    name: TRAIT_DEVELOPMENT_ENUM[v] ?? null,
    maddenName: MADDEN_DEV_TRAIT_ENUM[v] ?? null,
  };
}
// Accepts a raw enum value (0-4), a CFB tier name ('College_Star'), or a
// Madden dev trait name ('Superstar'). Both vocabularies resolve to the same
// byte, so reaching for the wrong one cannot write the wrong value.
function setDevTrait(player, trait) {
  const value = typeof trait === 'string' ? DEV_TRAIT_NAME_TO_VALUE[trait] : trait;
  if (!Number.isInteger(value) || !(value in TRAIT_DEVELOPMENT_ENUM)) {
    throw new Error(
      `setDevTrait: unrecognized trait ${JSON.stringify(trait)} `
      + `(expected 0-4, a CFB tier like "College_Star", or a Madden dev trait like "Superstar")`
    );
  }
  return setBinaryBytes(player, [{ offset: off(player, DEVTRAIT_OFFSET), value }]);
}
...
function getRatings(player) {
  const out = {};
  for (const [name, base] of Object.entries(RATING_OFFSETS)) out[name] = player.binary.raw[off(player, base)];
  return out;
}
