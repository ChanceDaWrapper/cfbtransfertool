...
async function extractLeavingPlayers(cfbSavePath, log = () => {}, opts = {}) {
  ...
  const forceSource = opts.forceSource ?? null; // 'leaving' | 'synthesized' | null(auto)
  ...
  const source = useLeaving ? 'leaving' : 'synthesized';

  await playerTable.readRecords();
  const statCache = new Map();

  // Build the selection list of { prec, projectRound, leaveType }, deduped by
  // player row (the game can list the same player in LeavingPlayer twice).
  const selection = [];
...
// Accessors so the athleticism z-score (used by dev traits) reads the
// calibrated player's Madden_-prefixed ratings + plain Height/Weight.
