...
// needs, and derives its int Rating* field list from that schema. Extracted
// out of extractLeavingPlayers (same open call, same error messages, same
// ratingFields derivation -- verbatim, not re-implemented) so Rosetta's
// calibration builder (Phase 5) can open an independent connection to the
// same save without duplicating the schema-override constants or the
...
// default) is the Power-Curve model engine; 'v1' is the legacy engine;
// 'rosetta' is the Two-Anchor engine (delegates to v1 in the live seam).
// Both concrete engines are injected here so lib/rosetta/ never has to
// require pipeline.js (the project's no-back-edge coupling rule).
