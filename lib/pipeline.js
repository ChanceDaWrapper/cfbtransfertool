...
//
// cfg.translation.strategy selects the implementation ('v1', the default --
// zero behavior change; 'rosetta' currently just delegates to 'v1' too,
// proving the seam before any real translation math exists).
function calibratePlayers(departedRows, { config, log = () => {} } = {}) {
  const cfg = mergeConfig(config);
  const strategy = cfg.translation?.strategy ?? 'v1';
  ...
  const translator = Rosetta.translation.createTranslator(strategy, { legacyCalibratePlayers: calibratePlayersV1 });
