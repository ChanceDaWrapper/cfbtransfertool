...
// POSITION in this same leaving class (not the whole league) -- computed
// once over the full pool, before the class-size/position-cap trimming
// below, so the reference group is as large as possible. Averaged across
// that position's traits and clamped so one freakish 99-speed outlier can't
// single-handedly dominate the draft order.
function computeAthleticismZScores(rows) {
  const byPosition = {};
  for (const r of rows) (byPosition[r.Position] ??= []).push(r);
  ...
  for (const [pos, group] of Object.entries(byPosition)) {
    ...
    const stats = traits.map((t) => {
      const vals = group.map((r) => Number(r[t]) || 0);
    ...
    for (const r of group) {
      const zs = stats.map((s) => (Number(r[s.trait]) - s.mean) / s.stdev);
...
function selectDeparted(rows, count, positionCaps, cfg) {
  ...
  const athleticZ = computeAthleticismZScores(rows);
