...
// The five ability/trait slots plus one more byte that only exist in Madden
// 27's wider record (see the module header). No M26 equivalent exists to
// carry these from, so -- exactly like gear -- they come from a POSITION-
// MATCHED donor in the target template rather than being left as whatever
// that template slot originally held.
const M27_ABILITY_BLOCK_OFFSETS = [202, 204, 206, 208, 210, 242];
...
function transferPlayer(dest, src, assigner, warn, { donor, validAssets, gearStats } = {}) {
  ...
  if (donor) {
    ...
    // position-matched donor's values ARE the whole answer, copied as raw
    // bytes rather than through any accessor since nothing in this file
    // format names what they mean.
    if (donor.binary.raw.length > Math.max(...M27_ABILITY_BLOCK_OFFSETS)) {
      try {
        p = setBinaryBytes(p, M27_ABILITY_BLOCK_OFFSETS.map((off) => ({ offset: off, value: donor.binary.raw[off] })));
