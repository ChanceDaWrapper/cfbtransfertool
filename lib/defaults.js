...
const SESSION_KEYS = ['population', 'league', 'translation', 'general', 'draftBoard'];
...
function defaultProfileTuning() {
  return {
    // Dice Roll engine settings (lib/rosetta/translation/diceRoll.js). Only
    // read when translation.strategy === 'diceroll'. The two fields serve
    // DIFFERENT leagues by design, not redundantly: the NFL profile reads
    // `classStrength` (roll-or-forced-tier, unchanged mechanism); the UFL
    // profile reads `debuff` (a fixed class-wide value, no roll -- see
    // calibratePlayersDiceRoll in pipeline.js). Both fields exist in both
    // profiles for shape symmetry (export/import, UI); each league's unused
    // field just sits inert.
    diceRoll: {
      // '' (default) = roll the class strength randomly each generation
      // (reproducible under the seed, one roll shared by the whole class).
      // Force a specific tier instead to always generate that strength of
      // class -- e.g. pinned to 'strong' for a deliberately easier class, or
      // for reproducing one exact result. (NFL profile only.)
      classStrength: '', // '' | 'veryWeak' | 'weak' | 'normal' | 'strong' | 'veryStrong'
      // Fixed class-wide debuff, no roll. (UFL profile only -- see
      // defaultUflProfile for its actual value; -0.20 here is just
      // "Normal"'s worth, inert until the NFL profile ever uses this field.)
      debuff: -0.20,
      // LEAGUE_PROFILES_ROADMAP.md Phase 3 (added the dial) / Phase 5 (chose
      // this default). Scales the per-player bust/gem roll (BUST_GEM_TABLE
      // in diceRoll.js -- roughly -0.10 to +0.06, rank-biased so early picks
      // lean positive and late picks lean negative) before it's added to the
      // class-wide debuff. 1.0 = the roll's full, unscaled swing (Dice
      // Roll's ORIGINAL behavior, still available by raising this back up).
      // 0 = no roll at all, every player gets exactly the class debuff.
      //
      // 0.3 shipped default -- measured against real Madden recomputes
      // (scoring actual written ratings through data/overall_formula.json,
      // NOT the EstMaddenOverall display, which undercounts Dice Roll's real
      // spread): at 1.0, a real import showed players in the 20s/30s/40s (up
      // to ~20% of a 402-player class under 50 overall on some rolls); 0.3
      // cuts that roughly in half while keeping real roll-to-roll player
      // variety (0 would make every player's rating purely a function of
      // their position/college overall, losing what makes this "Dice Roll").
      // Applies to BOTH leagues, independently -- this is a Dice Roll fix,
      // not a UFL-only one. NOTE: a rolled "Very Weak" class-strength tier
      // (NFL's classStrength left blank) still produces meaningfully more
      // sub-50 players than "Very Strong" even at this default (measured:
      // 87 vs 7 on the same save) -- that volatility comes from
      // CLASS_STRENGTH_DEBUFF's own severity spread (diceRoll.js, -25% to
      // -15%, unchanged from the original community-spreadsheet source) and
      // isn't something this per-PLAYER dial reaches; reworking that table
...
function defaultUflProfile() {
  ...
  // Stage 1, Dice Roll. -0.21 (Phase 3/4's value) was tuned to match Power
  // Curve's DISPLAYED baseline; the two engines' REAL baselines were
  // actually 16 points apart (53 vs 69) despite reading the same on screen.
  // -0.05 (with spread 0.3 below) closes that to 1 point (69 vs 70),
  // restoring the "engines agree at baseline" property Phase 3 intended.
  p.diceRoll = { ...p.diceRoll, debuff: -0.05, spread: 0.3 };
...
const DEFAULT_CONFIG = {
  ...
  // (Madden recomputes OVR in-game). 'diceroll' is a second, independently
  // selectable engine (lib/rosetta/translation/diceRoll.js): a per-class
  // strength roll and a per-player bust/gem roll drive one shared delta that
  // nudges every rating -- adapted from a community spreadsheet. 'v1' is the
  // legacy quantile/flat-drop + overall-anchor engine, kept for side-by-side
  // comparison. 'rosetta' is the Two-Anchor percentile engine (delegates to
  // v1 in the live seam for now). Shared across leagues -- flipping the
  // league toggle never switches engines (UFL_ROADMAP.md Decision 4).
  translation: {
...
const DESCRIPTIONS = {
  ...
  'translation.strategy': 'The method that turns college ratings into Madden ratings. Power Curve is deterministic and fully dial-driven -- every setting on this page and Rating Categories tunes it. Dice Roll is different: it rolls a strength for the whole class plus a bust/gem roll for each player, and everything flows from that -- the knobs below don\'t apply to it (see the Dice Roll card instead).',
  'diceRoll.classStrength': 'Only used by the Dice Roll engine, and only for NFL. Leave blank to roll a random class strength each time you generate (Very Weak through Very Strong, weighted toward the middle) -- still reproducible under your seed like everything else. Or force a specific tier to always generate a class of that exact strength. UFL has its own separate setting below instead of this one.',
  'diceRoll.debuff': 'Only used by the Dice Roll engine, and only for UFL. Sets a fixed starting strength for the class (same scale as the tiers above, e.g. -0.20 = Normal) instead of rolling one, so the UFL tier\'s baseline stays consistent every time you generate. NFL uses Class Strength above instead of this.',
  'diceRoll.spread': 'Only used by the Dice Roll engine. NFL and UFL each have their own copy of this. Controls how much each player\'s individual "bust or gem" luck can push them away from the class-wide strength. 1.0 = full swing, the original amount of luck. Lower it to keep every player closer to what the class strength alone would give them -- fewer extreme outliers. 0 = no luck at all, every player gets exactly the same result (no player-to-player variety). 0.3 is the shipped default -- noticeably tighter than full swing without going flat. Raise it above 1.0 for even wider luck.',
