...
const DEFAULT_CONFIG = {
  ...
  powerCurve: {
    ...
    // positionStrength. 1.0 = no-op (today's per-position values, unchanged).
  ...
  general: {
    classSize: 402,         // players pulled into the class; can never go below the Madden draft-class-file size (402)
...
const DESCRIPTIONS = {
  'population.mode': 'Engineering flag: how the departed-player pool is built. "legacy" is today\'s extraction. "exit" is the Rosetta Season Exit Population constructor, currently in validation.',
  'translation.strategy': 'Which rating-conversion engine runs. "powercurve" is the Power-Curve model (per-position strength dials, no OVR estimation). "v1" is the legacy quantile/flat-drop engine. "rosetta" is the Two-Anchor percentile engine.',
  'positionStrength.tech': 'How hard this position\'s TECHNICAL ratings (route running, coverage, blocking, pass rush, catching, throw accuracy) compress from college to rookie. Lower = keeps more of their college skill (stronger). Higher = bigger cut. 1.0 = full model compression.',
  'positionStrength.mental': 'How hard this position\'s MENTAL ratings (Awareness, Play Recognition) compress. This is always the biggest rookie drop. Lower = smarter rookies; higher = greener. Keep this above the Technical dial.',
  'positionStrength.physical': 'How hard this position\'s PHYSICAL ratings (including Throw/Kick Power) compress. The model keeps these near-identity (1.0). Lower = athleticism carries over even more intact.',
  'powerCurve.anchors': 'What percentage of a college rating survives the translation at this tier. Lower % = harsher compression for the whole category. The college reference value (Elite / Good) is fixed; only how much of it carries over is adjustable.',
  'powerCurve.globalStrength': 'One dial for the whole class: multiplies every position\'s Technical and Mental strength at once. Lower = the entire class keeps more college rating (stronger class). Higher = bigger cuts everywhere. 1.0 = today\'s per-position values, unchanged. Never touches Physical ratings.',
  ...
  'powerCurve.ratingCategory': 'Which compression bucket a rating uses. Physical barely changes; Technical (Light) compresses mildly; Technical (Heavy) more; Mental compresses hardest. Change this if a specific rating comes out too high or too low across the board.',
  ...
  'general.classSize': 'Players in the class. Can\'t go below 402 -- that\'s the fixed size of a Madden draft-class file.',
...
// Class Size can never go below 402 -- that's the fixed slot count in a
// Madden draft-class file (lib/draftClassExporter.js's TEMPLATE_SLOT_COUNT),
// and buildDraftClassFile hard-errors under it. Applied by ConfigStore (the
// user-facing config boundary) rather than inside mergeConfig itself, since
// pipeline.js also calls mergeConfig internally with intentionally small
// classSize overrides for fast, focused calibration tests.
const MIN_CLASS_SIZE = 402;
