...
function defaultPositionExtraDrop() {
  ...
  // reference data it was compensating for checked out fine on its own;
  // 0 (no override) was the correct neutral baseline. (That reference data
  // belonged to the V1 engine and went with it in 0.3.2; this dial stays --
  // Power Curve reads positionExtraDrop too.)
  return out;
}
...
// SESSION_KEYS below.
const TUNING_KEYS = [
  'diceRoll', 'powerCurve', 'overallBoost', 'positionStrength',
  'positionExtraDrop', 'positionCaps',
  'positionValue', 'draftValue', 'devTraits',
...
function defaultProfileTuning() {
  return {
    ...
    powerCurve: {
      ...
      // `ratingAdjustments`, which shipped a live AgilityRating cut for the V1
      // engine and would have silently activated under Power Curve. (That key
      // and the engine it belonged to were removed in 0.3.2.)
    ...
    // `legacy` and `bell` (the V1 engine's drop-leniency and bell-curve
    // squeeze) and `ratingAdjustments`/`kpAwarenessCap` (its per-rating drop
    // overrides and K/P awareness ceiling) were removed in 0.3.2 along with
    // the engine itself. Nothing reads them any more; a saved config that
    // still carries them simply drops them on the next merge, which is
    // harmless -- they never had a UI, so no user ever set one deliberately.
...
const DEFAULT_CONFIG = {
  ...
  // was replaced.)
  //
  // Two legacy strategies were removed in 0.3.2: 'v1' (the original
  // quantile/flat-drop + overall-anchor engine) and 'rosetta' (a percentile
  // "Two-Anchor" engine that was abandoned before any of its translation math
  // was written, leaving a strategy that delegated straight to v1). Neither was
  // ever reachable -- the engine picker has only ever offered the two above --
  // so a saved config naming one now falls back to 'powercurve' rather than
  // failing to open.
  //
  // Shared across leagues -- flipping the league toggle never switches engines
  // (UFL_ROADMAP.md Decision 4).
  translation: {
    strategy: 'powercurve', // 'powercurve' | 'diceroll'
  },
  general: {
...
// NOTE: there is deliberately no physicalHighlightAttributesFor(position)
// helper here. The renderer is the only consumer, it runs behind
// contextIsolation with no require() access to this module, and main.js hands
// it the two constants above over IPC -- so it does the concat itself (see
// renderer.js's rating-highlight code). A helper here could never be the one
// actually used, and the version that existed was dead for exactly that reason.
