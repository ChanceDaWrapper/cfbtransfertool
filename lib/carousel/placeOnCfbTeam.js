...
// (see placeOnTeam.js's synthesizeContractSalary, which this mirrors
// exactly). CFB's Coach schema DOES have a ContractSalary field (int,
// [0..16383], verified) -- it's not Madden-only, so this can't be skipped.
async function synthesizeCfbContractSalary(cfbFile, position, level) {
  const t = biggestTableByName(cfbFile, 'Coach');
  await t.readRecords();
  const peers = [];
  for (const r of t.records) {
...
async function planCfbTeamPlacement({ maddenCoachRecord, cfbFile, models, teamIndex, teamName, contractLength = 4, excludeRows = null, config = {} }) {
  ...
  // value, not a contradiction. synthesizeCfbContractSalary still runs (in
  // case a save genuinely does have salary data) but naturally returns 0
  // against a save like this one, which is exactly right.
  mapped.ContractSalary = await synthesizeCfbContractSalary(cfbFile, position, mapped.Level);
...
module.exports = {
  ...
  synthesizeCfbContractSalary, planCfbTeamPlacement,
};
