...
async function readNflCandidates(maddenFile, ctx) {
  ...
  for (const r of t.records) {
    ...
    const position = safe(r, 'Position');
    if (!HC_OC_DC.includes(position)) continue;
    // Blank filler shell -- the same exclusion readCfbCandidates has always
    // applied, which this reader was missing.
    //
    // A Madden Coach table carries rows that are not `isEmpty` but hold no
    // coach: no name, Level 0, no career, ContractStatus FreeAgent. The
    // sample save has exactly one (row 109). Without this guard the movement
    // engine happily proposed it as a transfer, and it scored WELL -- the
    // vulnerability scoring rewards a coach with no losses and no history,
    // so a blank shell looks like an unblemished free agent.
    //
    // Committing that move installs a nameless, level-0 coach into a real CFB
    // job and fires a genuine head coach to make room -- observed on a real
    // batch, where "" was hired at Maryland and Scott Satterfield displaced.
    // The job-security rank transfers correctly, so the permutation stays
    // intact and nothing downstream complains; the dynasty just quietly ends
    // up with an empty-named coach on the sideline.
    const level = safe(r, 'Level');
    if (!level) continue;
