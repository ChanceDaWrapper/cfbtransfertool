...
// The order categories are CLONED in, which is not cosmetic: createRowAllocator
// hands out free rows in claim order, so cloning Gameday before Playsheet puts
// each category's talents in the rows the other would otherwise have taken.
// The resulting save has the same row COUNTS and different row CONTENTS, and
// the coach's own references point somewhere else entirely.
//
// This bit me. Discovering categories from the schema yields them in schema
// order (Gameday, Playsheet, WearAndTear) where talentTree.js had always
// cloned them in this order (Playsheet, Gameday, WearAndTear) -- so simply
// swapping the hardcoded list for the discovered one silently changed every
// Madden 26 write, on a path that was supposed to be untouched. Caught by
// diffing a written save against one produced by the 0.2.3 build: six tables
// differed with identical occupancy.
//
// Must stay in sync with talentTree.js's TALENT_CATEGORIES.
const CANONICAL_CATEGORY_ORDER = ['PlaysheetTalents', 'GamedayTalents', 'WearAndTearTalents'];

// Category fields are exactly the reference fields whose declared type is a
// talent array -- true of Coach in M26 and of the holder row in M27. Ordered
// canonically (see above) so a discovered list clones identically to the
// hardcoded one it replaced; anything unrecognised keeps its schema order and
// follows the known ones.
function talentArrayFieldNames(schema) {
  if (!schema) return [];
  const found = schema.attributes.filter((a) => a.type === 'Talent[]').map((a) => a.name);
  const known = CANONICAL_CATEGORY_ORDER.filter((n) => found.includes(n));
  const unknown = found.filter((n) => !CANONICAL_CATEGORY_ORDER.includes(n));
...
module.exports = {
  ...
  talentArrayFieldNames, refOf, REQUIRED_SHARE, HOLDER_FIELD, CANONICAL_CATEGORY_ORDER,
};
