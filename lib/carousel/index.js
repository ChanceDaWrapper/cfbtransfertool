...
const { describeTalentLayout } = require('./maddenTalentLayout');
...
async function moveCoachCfbToMaddenTeam({
  ...
  if (!config.skipTalentTree) {
    // Where this save keeps talent categories, read off the save itself --
    // Madden 26 hangs them on the Coach record, Madden 27 on a holder row one
    // hop further in. See maddenTalentLayout.js.
    const layout = await describeTalentLayout(maddenFile);
