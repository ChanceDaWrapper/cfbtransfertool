...
async function buildCarouselModels({ cfbFile, maddenFile, winsor, direction = 'cfbToMadden' }) {
  ...
  // Madden 27 aliases OffensiveGuru as "First" at the same enum value, so a
  // name read off a coach record has to be canonicalized before it is
  // compared against anything. Built here, with the rest of the once-per-run
  // models, so both mappers share one map. See enumAlias.js.
  const maddenArchetypeAliases = enumAliasMap(maddenFile, { table: 'Coach', field: 'Archetype' });
  return { levelModel, cfbSchemeIndex, maddenSchemeIndex, direction, maddenArchetypeAliases };
}
...
async function mapCoachMaddenToCfb({ maddenCoachRecord, cfbFile, models, config = {} }) {
  ...
  out.DominantArchetype = mapArchetypeMaddenToCfb({
    // Canonicalized: on Madden 27 this field reads back as the alias "First"
    // for every OffensiveGuru coach, which matches no row in archetypeMap's
    // tables and would silently take the unrecognized-archetype fallback.
    archetype: canonicalEnumName(models.maddenArchetypeAliases, safe(maddenCoachRecord, 'Archetype')),
    level: out.Level, levelP70,
