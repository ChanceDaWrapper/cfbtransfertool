...
// Whether a table's own record layout matches the schema we hold for it.
//
// WHY THIS IS SEPARATE from schemaFieldNames. That function asks the SCHEMA
// LIST what fields a table should have, and the schema list is always the one
// Pipeline bundled -- so it answers "does our schema know about FirstName?"
// (always yes) rather than "can we actually read FirstName out of THIS save?".
// Those come apart when a title update changes a table.
//
// The real case that motivated this: a user's dynasty whose Coach table header
// declares 138 members while every CFB 27 schema in existence here declares
// 137. The franchise library will not bind a schema whose member count
// disagrees with the table header -- it silently substitutes a generic one
// derived from the header, whose fields are named Field_0..Field_137 and carry
// no types. Every read then returns undefined: 497 coaches present, 0 readable,
// every Position null. Nothing throws. The carousel just reports zero
// candidates (CFB->Madden) or "no disposable Coach row available" (Madden->CFB),
// both of which read as a bug in the tool or the user's dynasty rather than
// what it is -- a save newer than the schema we ship.
//
// Compared on member COUNT rather than by sniffing for Field_0 names, because
// the count is the thing the library itself binds on, so this reports exactly
// the condition that causes the fallback rather than a proxy for it.
function tableLayoutMismatch(file, tableName) {
  try {
    const table = biggestTableByName(file, tableName);
    const listed = file.schemaList.getSchema(tableName);
    if (!table || !listed || !listed.attributes) return null;
    const declared = table.header ? table.header.numMembers : undefined;
    if (typeof declared !== 'number') return null;
    const known = listed.attributes.length;
    return declared === known ? null : { declared, known };
  } catch (e) {
    return null;
  }
}

// The game build a save was written by, e.g. "College-27-RL3-9080944".
// Both games stamp it as plain text in the first bytes of the container,
// right after the FBCHUNKS magic -- the same convention the draft-class
// files use ("Madden-27-RL1-9074430"), and the only place a save says which
// TITLE UPDATE made it. `RL<n>` is the release number: a dynasty from CFB
// 27's third patch reads RL3, one from launch reads RL1.
//
// Read from the buffer already in memory rather than re-opening the path,
// so this cannot fail on a file that has since moved. Returns null rather
// than throwing for anything unexpected -- an unlabelled save is a worse
// error message, never a refusal on its own.
function saveBuildTag(file) {
  try {
    const buf = file.packedFileContents || file.unpackedFileContents;
...
function inspectCfbSave(cfbFile) {
  ...
  // Checked AFTER the field checks so a genuinely missing field is still named
  // specifically; this is the broader "we can't read this table at all" case.
  // Only Coach is fatal -- Team's layout has been observed to disagree by one
  // member on saves whose Coach table reads perfectly, and the carousel reads
  // only a handful of Team fields, so treating that as fatal would refuse
  // dynasties that work fine.
  const buildTag = saveBuildTag(cfbFile);
  for (const t of CFB_TABLES) {
    const mismatch = tableLayoutMismatch(cfbFile, t.name);
    if (!mismatch) continue;
    const msg = `this dynasty's "${t.name}" table has ${mismatch.declared} fields, but the `
      + `version of College Football 27 Pipeline supports has ${mismatch.known}. That almost `
      + 'always means the game has been updated since this build of Pipeline. Every value in '
      + `the table reads as blank as a result, and ${t.why} -- so there is nothing safe to read `
      + 'or write.'
      // Naming the build turns "some version mismatch" into something both the
      // user and whoever is triaging the report can act on -- it says exactly
      // which title update needs supporting.
      + (buildTag ? ` This dynasty was made by game build ${buildTag}.` : '')
      + ' Updating Pipeline should fix it, once it supports that build.';
    if (t.name === 'Coach') fatal.push(msg);
    else warnings.push(msg);
  }
