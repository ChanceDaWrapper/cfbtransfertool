...
// CFB's GenericHeadAssetName encodes skin tone AND a facial-hair-combo code in
// its own suffix ("Generic_0917_P_T0045_T_7_3" -> combo T, tone 7) -- a SEPARATE
// signal from PLYR_GENERICHEAD, and one this module used to throw away after
// reading only the tone digit out of it (extractSkinTone in pipeline.js).
//
// Ground-truthed against 429 real players who happen to carry BOTH head
// fields, so PLYR_GENERICHEAD's true category is known and the asset name's
// combo code can be checked against it directly:
//   H (n=89)  -> true category H,  100%
//   T (n=25)  -> true category T,  100%
//   M (n=51)  -> true category M,   98% (1 exception)
//   D (n=264) -> true category starts with B, 100% (B itself 75%; the rest
//                split across BMH/BHM/BTM/BM/BMT/BHTM -- every Madden combo
//                code that starts with "B" is in this family, and nothing
//                that starts with "B" is outside it, so "starts with" is an
//                exact partition, not an approximation)
// The skin digit matched exactly 429/429 -- already relied on via
// extractSkinTone, unchanged here.
//
// This is a genuinely different signal from PLYR_GENERICHEAD, not a weaker
// copy of it: CFB's own asset-name suffix vocabulary (H/T/M/D) never appears
// in Madden's combo vocabulary at all except by coincidence (there is no "D"
// category in Madden), which is WHY assignExact can't use it directly and why
// this needed its own family-based matcher instead of a literal lookup.
const CFB_ASSET_FAMILY = { H: 'H', T: 'T', M: 'M', D: 'B' };

// "Generic_0917_P_T0045_T_7_3" -> { tone: 7, family: 'T' }. Returns null for
// anything that doesn't match that shape -- most importantly Unique_* names
// (real scanned players), which carry no combo or tone signal at all and must
// fall through to the next tier rather than being handed a wrong answer.
function parseAssetName(assetName) {
  if (typeof assetName !== 'string') return null;
  const m = /_([A-Z]+)_(\d)_\d+$/.exec(assetName);
  if (!m) return null;
  const family = CFB_ASSET_FAMILY[m[1]];
  if (!family) return null; // an asset-name combo code this table hasn't seen
  return { tone: Number(m[2]), family };
}
...
function createAppearanceAssigner(catalogOverride) {
  ...
  // Second-best tier, tried when assignExact fails (Madden doesn't ship the
  // player's own head). Matches skin tone AND facial-hair family from CFB's
  // GenericHeadAssetName -- see CFB_ASSET_FAMILY above for why this beats a
  // skin-only substitute: a bearded skin-6 player currently gets ANY skin-6
  // head, clean-shaven ones included, when a same-tone same-family one is
  // sitting right there in the catalog.
  //
  // Deliberately does NOT widen the tone radius the way assign()'s
  // poolForTone does. If this exact (tone, family) pairing has no pairs, the
  // caller falls through to assign(skinTone), which already knows how to
  // widen -- returning a worse partial match here (right family, wrong tone)
  // instead of deferring to that would be a regression, not an improvement.
  //
  // Returns null when the asset name doesn't parse (Unique_* players, or
  // anything malformed) or the target game has no pair at that exact
  // (tone, family) -- either way the caller moves to the next tier.
  function assignByAssetName(assetName) {
    const parsed = parseAssetName(assetName);
    if (!parsed) return null;
    const bucket = catalog.bySkin[parsed.tone];
    if (!bucket || !bucket.pairs.length) return null;
    const candidates = bucket.pairs.filter((p) => {
      const m = /^gen_\d+_([A-Z]+)_/.exec(p.head);
      return m && m[1].startsWith(parsed.family);
    });
    ...
    return { faceId: best.faceId, head: best.head, tone: parsed.tone, exact: false, familyMatch: true };
  }
  ...
  return { assign, assignExact, assignByAssetName, stats, catalog };
}
...
module.exports = {
  ...
  // Exposed for direct unit testing of the family-matching rule itself,
  // independent of a full catalog/assigner.
  CFB_ASSET_FAMILY,
  parseAssetName,
};
